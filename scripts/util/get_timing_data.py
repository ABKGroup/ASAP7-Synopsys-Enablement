...
if __name__ == "__main__":
    ...
    df1["end_point"] = df1["points"].str.split().str[-1]
    df2["end_point"] = df2["points"].str.split().str[-1]
    ...
    if CASE == "1":
        ...
        matched_df = pd.merge(df1, df2, on=['end_point'], how='inner')

    if CASE != "all":
        cell_delays_x = np.concatenate(matched_df["cell_delays_x"].values)
        cell_delays_y = np.concatenate(matched_df["cell_delays_y"].values)

        r2_CD = plot(cell_delays_x, cell_delays_y, f"Cell Delay ({PEX1}-{STA1}) [ps]", f"Cell Delay ({PEX2}-{STA2}) [ps]", "Comp_of_Cell_delay", PLOT_DIR)
        mean_ad_CD, max_ad_CD = save_ad_dist(cell_delays_x, cell_delays_y, "AbsDiff [ps]", "Frequency", "Dist_of_AbsDiff_of_Cell_Delay", PLOT_DIR)
        ...
        r2_WD = plot(wire_delays_x, wire_delays_y, f"Wire Delay ({PEX1}-{STA1}) [ps]", f"Wire Delay ({PEX2}-{STA2}) [ps]", "Comp_of_Wire_delay", PLOT_DIR)
        mean_ad_WD, max_ad_WD = save_ad_dist(wire_delays_x, wire_delays_y, "AbsDiff [ps]", "Frequency", "Dist_of_AbsDiff_of_Wire_Delay", PLOT_DIR)
        ...
        r2_CS = plot(cell_slews_x, cell_slews_y, f"Cell Slew ({PEX1}-{STA1}) [ps]", f"Cell Slew ({PEX2}-{STA2}) [ps]", "Comp_of_Cell_slew", PLOT_DIR)
        mean_ad_CS, max_ad_CS = save_ad_dist(cell_slews_x, cell_slews_y, "AbsDiff [ps]", "Frequency", "Dist_of_AbsDiff_of_Cell_Slew", PLOT_DIR)
        ...
        r2_WS = plot(wire_slews_x, wire_slews_y, f"Wire Slew ({PEX1}-{STA1}) [ps]", f"Wire Slew ({PEX2}-{STA2}) [ps]", "Comp_of_Wire_slew", PLOT_DIR)
        mean_ad_WS, max_ad_WS = save_ad_dist(wire_slews_x, wire_slews_y, "AbsDiff [ps]", "Frequency", "Dist_of_AbsDiff_of_Wire_Slew", PLOT_DIR)
    ...
    r2_A = plot(arrival_x, arrival_y, f"Arrival ({PEX1}-{STA1}) [ps]", f"Arrival ({PEX2}-{STA2}) [ps]", "Comp_of_Arrival", PLOT_DIR)
    mean_ad_A, max_ad_A = save_ad_dist(arrival_x, arrival_y, "AbsDiff [ps]", "Frequency", "Dist_of_AbsDiff_of_Arrival", PLOT_DIR)
...
#
#    r2_R = plot(required_x, required_y, f"Required Time ({PEX1}-{STA1}) [ps]", f"Required Time ({PEX2}-{STA2}) [ps]", "Comp_of_Required", PLOT_DIR)
#    mean_ad_R, max_ad_R = save_ad_dist(required_x, required_y, "AbsDiff [ps]", "Frequency", "Dist_of_AbsDiff_of_Required", PLOT_DIR)
#
...
#    min_R = np.min(requireds)
    ...
    r2_S = plot(slack_x, slack_y, f"Slack ({PEX1}-{STA1}) [ps]", f"Slack ({PEX2}-{STA2}) [ps]", "Comp_of_Slack", PLOT_DIR)
    mean_ad_S, max_ad_S = save_ad_dist(slack_x, slack_y, "AbsDiff [ps]", "Frequency", "Dist_of_AbsDiff_of_Slack", PLOT_DIR)
